...
{ 
    ...
    if ($connex) 
    {
        ...
        $requeteKeywords = "SELECT Nom , Id_Keyword FROM keywords WHERE Id_Keyword = ".$keywords.";";
        ...
        if (!$resultArticle) 
        ...
        else 
            {
                ...
                foreach ($articles as $a)
                {
                    ...
                    $requeteCategorie = "SELECT c.Nom , c.Id_Categorie FROM categorie c , article a WHERE a.Id_Categorie = c.Id_Categorie AND a.Id_Article =".$a[0].";";
                    $resultCategorie = mysqli_query($connex,$requeteCategorie);
                    $categorie = mysqli_fetch_array($resultCategorie);

                    $requeteAllKeywords = "SELECT k.Nom , k.Id_Keyword FROM keywords k , article_keywords ak , article a WHERE a.Id_Article =".$a[0]." AND a.Id_Article = ak.Id_Article AND ak.Id_Keyword = k.Id_Keyword" ;
